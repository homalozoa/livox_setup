...
if [ -z "$IFACE_IP" ]; then
    echo "[ERROR] $IFACE 没有 IP，请先运行 setup_network.sh"
    exit 1
fi

SUBNET=$(echo "$IFACE_IP" | cut -d. -f1-3)
echo "[INFO] 扫描 ${SUBNET}.0/24 网段寻找 Livox 雷达..."

# 方法1: 抓取雷达广播包（最快，2秒内）
LIDAR_IPS=$(timeout 3 sudo tcpdump -i "$IFACE" -c 5 -n 2>/dev/null \
    | grep -oP '\d+\.\d+\.\d+\.\d+' \
    | sort -u \
    | grep -v "$IFACE_IP" \
...
if command -v arp-scan &>/dev/null; then
    ...
    # 方法3: ping 扫描（最慢）
    echo "[INFO] 使用 ping 扫描（可能需要 30 秒）..."
    for i in $(seq 1 254); do
        ping -c 1 -W 1 -I "$IFACE" "${SUBNET}.$i" &>/dev/null &
    done
    wait
    LIDAR_IPS=$(arp -i "$IFACE" -n 2>/dev/null \
        | grep -oP '\d+\.\d+\.\d+\.\d+' \
...
if [ -n "$LIDAR_IPS" ]; then
    ...
    echo "[WARN] 未发现雷达，请检查:"
    echo "  1. 雷达是否通电"
    echo "  2. 网线是否连接到 $IFACE"
    echo "  3. 雷达 IP 是否在 ${SUBNET}.0/24 网段"
    exit 1
fi
